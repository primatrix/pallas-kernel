...
from tops.utils import assert_shape_or_none, assert_shape
...
def chunk_simple_gla_fwd(
  ...
  *,
  ...
  """Simple GLA chunked forward.

  Args:
    q: [B, T, H, K]
    k: [B, T, H, K]
    v: [B, T, H, V]
    g: [B, T, H] or None — per-step scalar gate (already chunk-local cumsummed)
    g_gamma: (H,) or None — fixed per-head log decay
    scale: scaling factor (default K**-0.5)
    h0: [B, H, K, V] or None — initial state
    use_ht: whether to return final state
    cu_seqlens: [B+1] or None — variable-length sequence boundaries
    chunk_size: chunk size

  Returns:
    (o, ht) — output [B, T, H, V], final state [B, H, K, V] or None
  """
  B, T, H, K, V = *q.shape, v.shape[-1]
  N = B if cu_seqlens is None else cu_seqlens.shape[0] - 1

  assert_shape([q, k], (B, T, H, K))
  assert_shape([v], (B, T, H, V))
  assert_shape_or_none(g, (B, T, H))
  assert_shape_or_none(g_gamma, (H,))
  assert_shape_or_none(h0, (N, H, K, V))
  assert_shape_or_none(cu_seqlens, (B + 1,))
  assert T % chunk_size == 0
...
def chunk_simple_gla_bwd(
  ...
  *,
  ...
  Args:
    ...
    v: [B, T, H, V]
    do: [B, T, H, V] — output gradient
    dht: [N, H, K, V] or None — final state gradient
    g: [B, T, H] or None — per-step scalar gate (already chunk-local cumsummed)
    g_gamma: (H,) or None — fixed per-head log decay
    h0: [B, H, K, V] or None — initial state
    scale: scaling factor (default K**-0.5)
